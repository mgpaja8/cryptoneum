...
export function loadingReducer<T, K extends keyof T>(
  key: K
...
export function successReducer<T, K extends keyof T, V>(
  key: K
...
export function errorReducer<T, K extends keyof T, E>(
  key: K
...
export function valueReducer<T, K extends keyof T>(
  key: K
...
export function mapReducersArray<T>(
  ...
  const reducerMap = (reducerArray || []).reduce((
    acc: Map<string, ActionReducer<T>>,
    result: ReducerArrayType<T>
  ) => {
    if (Array.isArray(result[0])) {
      (result as AsyncReducerResult<T>).forEach(asyncResult => {
        acc.set(asyncResult[0], asyncResult[1]);
      });
    } else {
      acc.set((result as ReducerResult<T>)[0], (result as ReducerResult<T>)[1]);
    }
    return acc;
